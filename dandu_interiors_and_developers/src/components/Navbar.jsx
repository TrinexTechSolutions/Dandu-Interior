...
const Navbar = () => {
  ...
  return (
    <nav 
      ...
      className="fixed top-0 left-0 w-full z-50 py-6 transition-colors duration-300 bg-[#F8F5F2] border-b border-[#1A1A1A]/5"
    >
        ...
        <div className="flex justify-between items-center relative">
          ...
          <div className="hidden lg:flex items-center justify-center gap-2 absolute left-1/2 -translate-x-1/2 w-max pointer-events-auto z-20">
            {[
              ...
              <NavLink 
                ...
                className={({ isActive }) => `text-[10px] font-bold tracking-[0.25em] uppercase px-4 py-2 transition-all duration-300 ${
                  isActive 
                  ? 'text-[#1A1A1A]' 
            ...
            >
              <button 
                ...
                className={`flex items-center gap-2 text-[10px] font-bold tracking-[0.25em] uppercase px-4 py-2 transition-all duration-300 ${
              ...
              <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-5 transition-all duration-300 origin-top z-50 ${servicesOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}>
                <div className="w-[520px] bg-[#F8F5F2] shadow-2xl border border-[#1A1A1A]/10 p-6">
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                    {SERVICES.map((service, idx) => (
                      <NavLink 
                        key={idx} 
                        to={service.path}
                        className={({ isActive }) => `flex items-center gap-4 p-3 transition-colors group/item ${
                          isActive 
                          ? 'text-[#1A1A1A]' 
                          : 'text-[#1A1A1A]/40 hover:text-[#1A1A1A]'
                        }`}
          ...
          <div className="flex items-center justify-end flex-1 lg:flex-none gap-5 z-10 relative">
            ...
            <div className="hidden lg:flex items-center gap-5">
              <a href="https://wa.me/919866166612" target="_blank" rel="noreferrer" className="text-[#1A1A1A] hover:opacity-70 transition-opacity" aria-label="WhatsApp">
              ...
              <a href="#" className="text-[#1A1A1A] hover:opacity-70 transition-opacity" aria-label="Instagram">
              ...
              <a href="#" className="text-[#1A1A1A] hover:opacity-70 transition-opacity" aria-label="Pinterest">
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden absolute top-full left-0 w-full bg-[#F8F5F2] border-t border-[#1A1A1A]/5 shadow-xl transition-all duration-300 origin-top overflow-y-auto ${isOpen ? 'max-h-[calc(100vh-80px)] py-8 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="w-full px-6 flex flex-col gap-6 pb-12">
          {[
            ...
            <NavLink 
              ...
              className={({ isActive }) => `text-[10px] font-bold tracking-[0.3em] uppercase p-2 transition-colors ${
                isActive 
                ? 'text-[#1A1A1A]' 
                : 'text-[#1A1A1A]/40'
              }`}
          ...
          <div className="flex flex-col">
            <button 
              ...
              className={`flex justify-between items-center w-full text-[10px] font-bold tracking-[0.3em] uppercase p-2 text-left transition-colors ${
                location.pathname.startsWith('/services')
                ? 'text-[#1A1A1A]'
                : 'text-[#1A1A1A]/40'
              }`}
            ...
            <div className={`flex flex-col overflow-hidden transition-all duration-300 ${mobileServicesOpen ? 'max-h-[600px] mt-4' : 'max-h-0'}`}>
              <div className="pl-4 border-l border-[#1A1A1A]/10 ml-2 py-2 flex flex-col gap-2">
                {SERVICES.map((service, idx) => (
                  <NavLink 
                    key={idx} 
                    to={service.path}
                    onClick={() => setIsOpen(false)}
                    className={({ isActive }) => `text-[10px] font-medium tracking-wider uppercase p-2 transition-colors ${
                      isActive 
                      ? 'text-[#1A1A1A]' 
                      : 'text-[#1A1A1A]/40'
                    }`}
